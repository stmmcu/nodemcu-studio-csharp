...
namespace Tester
{
    public partial class Mainform : Form
    {
        ...
        public Mainform()
        {
            StartingUp OStUp = new StartingUp();
            OStUp.Show();

            InitializeComponent();

            keywords = file.Instance.fileLDLine("keyword.setting");//{ "abstract", "as", "base", "bool", "break", "byte", "case", "catch", "char", "checked", "class", "const", "continue", "decimal", "default", "delegate", "do", "double", "else", "enum", "event", "explicit", "extern", "false", "finally", "fixed", "float", "for", "foreach", "goto", "if", "implicit", "in", "int", "interface", "internal", "is", "lock", "long", "namespace", "new", "null", "object", "operator", "out", "override", "params", "private", "protected", "public", "readonly", "ref", "return", "sbyte", "sealed", "short", "sizeof", "stackalloc", "static", "string", "struct", "switch", "this", "throw", "true", "try", "typeof", "uint", "ulong", "unchecked", "unsafe", "ushort", "using", "virtual", "void", "volatile", "while", "add", "alias", "ascending", "descending", "dynamic", "from", "get", "global", "group", "into", "join", "let", "orderby", "partial", "remove", "select", "set", "value", "var", "where", "yield" };
            methods = file.Instance.fileLDLine("methods.setting");//{ "Equals()", "GetHashCode()", "GetType()", "ToString()" };
            snippets = file.Instance.fileLDLine("snippets.setting");//{ "if(^)\n{\n;\n}", "if(^)\n{\n;\n}\nelse\n{\n;\n}", "for(^;;)\n{\n;\n}", "while(^)\n{\n;\n}", "do\n{\n^;\n}while();", "switch(^)\n{\ncase : break;\n}" };
            declarationSnippets = file.Instance.fileLDLine("\\declarationSnippets.setting");
            ...
            InitCOM F_OIntCom = new InitCOM();
            F_OIntCom.ShowDialog();

            if (keywords == null || methods == null || snippets == null || declarationSnippets == null)
                this.StatusLabel1.Text = "Setting File Load Error...";
            try
            {
                if (!COM.Instance.Com.IsOpen)
                {
                    StatusLabel1.Text += "Serial Port Connect Fail";
                }
                else
                {
                    StatusLabel1.Text += "Serial Port Connect Success";
                }
            }
            catch
            {
                StatusLabel1.Text += "Skip Serial Port Connection";
            }
        ...
        private void CreateTab(string fileName)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    if (MessageBox.Show(ex.Message, "Unexception Error", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) == DialogResult.Retry)
                        CreateTab(fileName);
                }
            }
        }
        ...
        private void DownloadButton1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!COM.Instance.Com.IsOpen)
                {
                    ...
                    InitCOM react = new InitCOM(false);

                    react.ShowDialog();

                    if (!COM.Instance.Com.IsOpen)
                    {
                        MessageBox.Show("Unload Error...", "USER CANCELED");
                        return;
                    }
                    else
                    {
            ...
            if (tsFiles.SelectedItem != null)
            {
                ...
                {
                    Window1 download = new Window1();

                    download.Show();

                    download._download(tsFiles.SelectedItem.Tag as string);
            ...
            {
                MessageBox.Show("No Document Selected" , "Mindless ERROR");
                CreateTab(null);
            }
